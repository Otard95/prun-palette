...
const isPaletteCommandVariable = (
  value: unknown
): value is PaletteCommandVariables => {
  return Object.values(PaletteCommandVariables).includes(
    value as PaletteCommandVariables
  )
}
...
export default class Palette {
  ...
  @memoize()
  private signatureStartsWith(
    sig: CommandSignature,
    prefix: CommandSignature
  ): boolean {
    if (sig.length < prefix.length) return false
    for (let i = 0; i < prefix.length; i++) {
      if (
        isPaletteCommandVariable(prefix[i]) ||
        isPaletteCommandVariable(sig[i])
      )
        continue
  ...
  private getCommandsStartingWithSig(
    partialSig: CommandSignature
  ): PaletteCommand[] {
    return this.commands.filter(command =>
      this.signatureStartsWith(command.signature, partialSig)
    )
  }

  private getCommandMatchingSig(sig: CommandSignature): PaletteCommand | null {
    const command = this.commands.find(command =>
      this.signaturesMatches(sig, command.signature)
    )
    return command || null
  }
  ...
  public sortedMatches(
    ...
    const matchesWithVariables = partialMatches.filter(command =>
    ...
    const nextArgs = partialCommands.map(
      command => command.signature[partialSig.length]
    )

    const scoredCommands = matchArrays(
      partialCommands,
      fuzzStrings(input, nextArgs)
    )
      .filter(
        (match): match is [PaletteCommand, number] => match[1] !== undefined
      )
      .sort((a, b) => b[1] - a[1])
  ...
  private isFuzzable(sigPart: string | PaletteCommandVariables): boolean {
    ...
    return (
      typeof sigPart === 'string' &&
      (!isPaletteCommandVariable(sigPart) ||
        fuzzableVariables.includes(sigPart))
    )
  }
  ...
  public fuzzyNextArg(
    ...
    const partialMatches = this.getCommandsStartingWithSig(partialSig).filter(
      command => {
        return (
          command.signature.length > partialSig.length &&
          this.isFuzzable(command.signature[partialSig.length])
        )
      }
    )
